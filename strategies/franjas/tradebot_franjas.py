...
def parse_pct_list(raw: str, flag: str) -> list[float]:
    """Convierte '3,5,7' en [0.03, 0.05, 0.07]."""
    try:
        vals = [float(v.strip()) / 100 for v in raw.split(",") if v.strip()]
    except ValueError:
        raise SystemExit(f"Error: {flag} inválido ({raw}); usá números separados por coma, ej. 3,5,7")
    if not vals or any(v <= 0 for v in vals):
        raise SystemExit(f"Error: {flag} requiere valores positivos, ej. 3,5,7")
    return vals


def band_of(drawdown: float, band_bounds: list[float]) -> int:
    """Franja según la caída acumulada desde la referencia del ciclo.
    Franja 0: caída < bounds[0]; última franja: caída >= bounds[-1]."""
...
def setup_logging(symbol: str):
    log_file = f"tradebot_franjas_{symbol}.log"
    logging.basicConfig(
        level=logging.INFO,
    ...
    if os.path.exists(state_file):
        try:
            with open(state_file, "r", encoding="utf-8") as f:
                ...
                if "cycle_ref" not in state:
                    state["cycle_ref"] = None
                return state
        except Exception:
            logging.exception(f"Error al leer {state_file}. Se iniciará estado vacío.")
    return {"purchases": [], "profit_pool": 0.0, "cycle_ref": None}
...
def ensure_franja_fields(state: dict, rises: list[float], band_bounds: list[float]) -> dict:
    """Garantiza que el estado tenga cycle_ref y que cada lote tenga franja y
    rise propio. Tras una reconciliación (lotes reconstruidos desde Alpaca)
    la referencia del ciclo se toma como el precio del lote más antiguo
    abierto y la franja de cada lote se recalcula desde ahí."""
    purchases = state.get("purchases", [])
    if not purchases:
        state["cycle_ref"] = None
        return state
    if not state.get("cycle_ref"):
        state["cycle_ref"] = purchases[0]["price"]
        logging.info(f"cycle_ref reconstruido: ${state['cycle_ref']:.2f} (primer lote abierto)")
    for p in purchases:
        if "rise" not in p or "band" not in p:
            band = band_of(1.0 - p["price"] / state["cycle_ref"], band_bounds)
            p["band"] = band
            p["rise"] = rises[band]
            logging.info(f"Lote a ${p['price']:.2f}: franja {band} asignada (rise {p['rise']*100:.1f}%)")
    return state
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Bot de Grid Trading con drop/rise por franjas de caída")
    parser.add_argument("--symbol",     type=str,   default=None)
    parser.add_argument("--buy-amount", type=float, default=None)
    parser.add_argument("--max-buys",   type=int,   default=None)
    parser.add_argument("--drops",      type=str,   default=None, help="Drop %% por franja, ej. 2,2,2")
    parser.add_argument("--rises",      type=str,   default=None, help="Rise %% por franja, ej. 3,5,7")
    parser.add_argument("--bands",      type=str,   default=None, help="Límites de franja en %% de caída desde la referencia del ciclo, ej. 6,10")
    ...
    drops       = parse_pct_list(get(args.drops, "DROPS", str, "2,2,2"),  "--drops/DROPS")
    rises       = parse_pct_list(get(args.rises, "RISES", str, "3,5,7"),  "--rises/RISES")
    band_bounds = parse_pct_list(get(args.bands, "BANDS", str, "6,10"),   "--bands/BANDS")
    interval    = get(args.interval, "INTERVAL", int, 1200)

    if not (len(drops) == len(rises) == len(band_bounds) + 1):
        print(f"Error: se requieren N franjas con N drops y N rises y N-1 límites. "
              f"Recibido: {len(drops)} drops, {len(rises)} rises, {len(band_bounds)} límites.")
        return
    if sorted(band_bounds) != band_bounds:
        print(f"Error: los límites de franja deben ser crecientes: {[b*100 for b in band_bounds]}")
        return
    ...
    logging.info(f"=== Iniciando Grid Bot FRANJAS: {symbol} ===")
    logging.info(f"  buy_amount : ${buy_amount:,.2f}")
    logging.info(f"  max_buys   : {max_buys}")
    logging.info(f"  drops      : {'/'.join(f'{d*100:g}%' for d in drops)}")
    logging.info(f"  rises      : {'/'.join(f'{r*100:g}%' for r in rises)}")
    logging.info(f"  franjas    : caída < {band_bounds[0]*100:g}% / hasta {band_bounds[-1]*100:g}% / más")
    ...
    state_file = os.path.join(data_dir, f"tradebot_franjas_{symbol}_state.json")
    ...
    if reconciled is not state:
        state = reconciled
    state = ensure_franja_fields(state, rises, band_bounds)
    ...
    while True:
        try:
            ...
            state     = ensure_franja_fields(load_state(state_file), rises, band_bounds)
            purchases = state["purchases"]
            ...
            if len(purchases) == 0:
                ...
                if buy_info:
                    ...
                    state["cycle_ref"]   = buy_info["price"]
                    buy_info["band"] = 0
                    buy_info["rise"] = rises[0]
                    purchases.append(buy_info)
                    save_state(state, state_file)
                    logging.info(f"Grid iniciado. Compra a ${buy_info['price']:.2f} (referencia del ciclo). Pool restante: ${state['profit_pool']:.2f}")
                else:
            ...
            cycle_ref      = state["cycle_ref"]
            band_last      = band_of(1.0 - last_buy_price / cycle_ref, band_bounds)
            buy_target     = last_buy_price * (1.0 - drops[band_last])
            sell_target    = last_buy_price * (1.0 + last_purchase["rise"])

            logging.info(f"-> Última compra: ${last_buy_price:.2f} (franja {band_last}) | Referencia ciclo: ${cycle_ref:.2f} | Activas: {len(purchases)}/{max_buys} | Pool: ${state.get('profit_pool', 0.0):.2f}")
            logging.info(f"-> Objetivo COMPRA: ${buy_target:.2f} (drop {drops[band_last]*100:g}%) | Objetivo VENTA: ${sell_target:.2f} (rise {last_purchase['rise']*100:g}%)")
            ...
            if current_price <= buy_target:
                if len(purchases) < max_buys:
                    ...
                    if buy_info:
                        state["profit_pool"] = pool - bonus
                        band = band_of(1.0 - buy_info["price"] / cycle_ref, band_bounds)
                        buy_info["band"] = band
                        buy_info["rise"] = rises[band]
                        purchases.append(buy_info)
                        save_state(state, state_file)
                        logging.info(f"Compra registrada a ${buy_info['price']:.2f} (franja {band}, rise {rises[band]*100:g}%). Activas: {len(purchases)}. Pool restante: ${state['profit_pool']:.2f}")
                    else:
                ...
                if sell_info:
                    ...
                    if purchases:
                        logging.info(f"Referencia regresa a: ${purchases[-1]['price']:.2f} (rise {purchases[-1]['rise']*100:g}%)")
                    else:
                        logging.info("Todos los lotes vendidos. Grid (y referencia del ciclo) se reinicia en el próximo ciclo.")
                else:
